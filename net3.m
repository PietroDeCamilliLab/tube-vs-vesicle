...
pause(0.00001);
index=1;
[filename,pathname]=uigetfile({'*.tif;*.jpg;*.png','(*.tif;*.jpg;*.png)';'*.tif','(*.tif)';'*.jpg','(*.jpg)';'*.png','(*.png)';},'打开图片');
if filename==0
    return;
...
% figure,imshow(BW_si );title('si');
  str1=strcat('总面�?:�?',num2str(area_si,'%d'),'�?',num2str(area_pao,'%d'), '比率:' ,num2str(area_si/area_pao*100,'%.2f'),'%  数量：丝',num2str(num_si),'�?',num2str(num_pao), '比率:' ,num2str(num_si/num_pao*100,'%.2f'),'%');
